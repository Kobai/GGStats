...
@app.route('/')
def index():
    return redirect(url_for('all_stats', floor=99))
...
def all_stats(floor):
    ...
    wr_fig.update_layout(title=f'Floor {floor} Win Rates', font=dict(size=14))
    ...
    pr_fig.update_layout(title=f'Floor {floor} Play Rates', font=dict(size=14))
    ...
    mu_fig.update_layout(title=f'Floor {floor} Match Up Chart', font=dict(size=14))
